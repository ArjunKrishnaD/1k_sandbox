...
namespace Engine.Tests.Unit
{
    public class SimpleProcedureTest
    {
        ...
        public void Success()
        {
            ...
            Assert.Equal(ProcedureStatus.SUCCESS, procedure.Execute(new ProcedureLog()));
        ...
        public void Failure()
        {
            ...
            SimpleProcedure procedure = new SimpleProcedure(task);
            Assert.Equal(ProcedureStatus.FAILURE, procedure.Execute(new ProcedureLog()));
            Assert.Equal(0, task.ExecutedCount);
            Assert.Equal(1, task.FailedCount);
            Assert.Throws<InvalidOperationException>(() => procedure.Execute(new ProcedureLog()));
        }
        ...
        public void Suspension()
        {
            ...
            SimpleProcedure procedure = new SimpleProcedure(task);
            Assert.Equal(ProcedureStatus.SUSPEND, procedure.Execute(new ProcedureLog()));
            Assert.Equal(0, task.ExecutedCount);
            Assert.Equal(0, task.FailedCount);
